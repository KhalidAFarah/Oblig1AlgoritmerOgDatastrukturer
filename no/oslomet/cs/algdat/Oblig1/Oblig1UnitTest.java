...
class Oblig1UnitTest {
    ...
    @org.junit.jupiter.api.Test
    void gjennomsnittligeOmbyttninger() {
        int n = 100;
        int[][] a = new int[n][];
        for(int i = 0; i < a.length; i++){
            a[i] = Oblig1Test.randPerm(1000);
        }
        int ombyttinger = 0;
        for(int[] ints : a){
            ombyttinger += Oblig1.ombyttinger(ints);
        }

        int gjennomsnittligeOmbyttinger = ombyttinger/a.length;
        System.out.println("gjennomsnittlige ombyttninger for " + n + " arrays er:");
        System.out.println(gjennomsnittligeOmbyttinger);
    }
}